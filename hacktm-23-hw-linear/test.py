from half import *
import itertools
import solve

# from pwn import enhex
def is_zero_test():
  for i in range(256):
    expr = is_zero(x())
    r = half_linear_eval(expr, half_of_int(i))
    if i == 0:
      assert r == half_of_int(1)
    else:
      assert r == half_of_int(0)

def eq_test():
  for i, j in itertools.product(range(256), range(256)):
    expr = eq(x(), const_int(j))
    r = half_linear_eval(expr, half_of_int(i))
    if i == j:
      assert r == half_of_int(1), f'{i} {j} {r}'
    else:
      assert r == half_of_int(0), f'{i} {j} {r}'

def if_test():
  for i in range(256):
    expr = build_if(const_int(1), x())
    r = half_linear_eval(expr, half_of_int(i))
    assert r == half_of_int(i)
    
    expr = build_if(const_int(0), x())
    r = half_linear_eval(expr, half_of_int(i))
    assert r == half_of_int(0)
    


def sbox_test(expr):
  s_box = (
    ,
  )
  
 
  for i in range(256):
    r = half_linear_eval(expr, half_of_int(i))
    assert r == s_box[i], f'{r} != {s_box[i]}'

def prefix_test(expr):
  assert expr_of_prefix(expr_to_prefix(expr)) == expr
if __name__ == '__main__':
  # is_zero_test()
  # print('is_zero_test passed')
  # eq_test()
  # print('eq_test passed')
  # if_test()
  # print('if_test passed')
  # sbox_test(solve.sbox_atempt2())
  # print('sbox_test passed')
  # prefix_test(solve.sbox_atempt1())
  # print('prefix_test passed')
  expr = solve.sbox_atempt2()
  print(expr)
  data = serialize(expr)
  # print(data)
  # print(len(data))
  new_expr = deserialize(data)
  assert new_expr == expr
  

